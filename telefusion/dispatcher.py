...
from telefusion.handlers import (
...
from telefusion.raw.types import (
...
class Dispatcher:
    ...
    def __init__(self, client: "telefusion.Client"):
        ...
        async def message_parser(update, users, chats):
            return (
                await telefusion.types.Message._parse(self.client, update.message, users, chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await telefusion.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                telefusion.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                telefusion.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                telefusion.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                telefusion.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                telefusion.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                telefusion.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ChatJoinRequestHandler
            )

        async def story_parser(update, users, chats):
            return (
                await telefusion.types.Story._parse(self.client, update.story, update.peer),
                StoryHandler
            )
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except telefusion.StopPropagation:
                                raise
                            except telefusion.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
